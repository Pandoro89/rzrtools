class ExportLastMonthsCsvJob
  ...
  def self.perform()
    ...
    CSV.open(file_path, "w", {:force_quotes => true}) do |csv|
      ...
      FleetPosition.where("created_at >= ? AND created_at < ? AND fleet_id NOT IN (SELECT id FROM fleets WHERE status > 1) AND alliance_id = ?", last_month_date, Date.today.beginning_of_month, ALLIANCE_ID).each {|fp|
    ...
    CSV.open(file_path, "w", {:force_quotes => true}) do |csv|
      ...
      FleetPosition.where("special_role != 'none' AND special_role IS NOT NULL AND created_at >= ? AND created_at < ? AND fleet_id NOT IN (SELECT id FROM fleets WHERE status > 1) AND alliance_id = ?", last_month_date, Date.today.beginning_of_month, ALLIANCE_ID).each {|fp|
